...
median vals = if length vals `mod` 2 == 1
        then mean [last (take (halflength + 1) vals)]
        else mean [lowerhalf , upperhalf]
    where halflength = div (length vals) 2
...
movingMed [] n = []
movingMed vals n = if length nextVals == n
...
sampleVar sample = sum terms % length sample
    where terms = map (\x -> (x - mean sample)^2) sample

varMaybe :: [Maybe Double] -> Maybe Double
varMaybe sample = if any (== Nothing) sample
                then Nothing
                else (Just var)
    where var = sampleVar (map fromJust sample)